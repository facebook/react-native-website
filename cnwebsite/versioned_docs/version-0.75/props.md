...
上面的例子出现了一样新的名为[`View`](view)的组件。[`View`](view) 常用作其他组件的容器，来帮助控制布局和样式。

仅仅使用`props`和基础的[`Text`](text)、[`Image`](image)以及[`View`](view)组件，你就已经足以编写各式各样的 UI 组件了。要学习如何动态修改你的界面，那就需要进一步[学习 State（状态）的概念](state)。
