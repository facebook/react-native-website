...
`<ScrollView>` vs `<ListView>` - which one to use? ScrollView renders all its react child components at once, but this has a performance downside. On the other hand, this has a performance downside. Imagine you have a very long list of items you want to display, worth of couple of your ScrollView’s heights. Creating JS components and native views upfront for all its items, which may not even be shown, will contribute to slow rendering of your screen and increased memory usage.
