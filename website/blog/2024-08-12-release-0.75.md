---
title: 'React Native 0.75 - Frameworks, Yoga 3.1, New Architecture updates, and more'
authors:
  [
    gabrieldonadel,
    siddharthkul,
...
This release ships several features, such as the introduction of the recommendation for users to use a React Native Framework, Yoga 3.1 with support for % parameters, and several stabilization fixes for the New Architecture.

### Highlights

- [React Native Frameworks](/blog/2024/08/12/release-0.75#react-native-frameworks)
...
- [Last version supporting minSdk 23 and minIOSVersion 13.4](/blog/2024/08/12/release-0.75#last-version-supporting-minsdk23-and-miniosversion-134)
...
Version 0.75 brings several changes to React Native that reflect those recommendations:

- We moved the `/template` folder from the `react-native` package to a separate repo called `react-native-community/template`
...
Historically, `react-native` used to ship a `/template` folder inside the NPM package which was used by the Community CLI to create new projects.
...
This change brings the template closer to the Community CLI and will make it easier for everyone to inspect and depend on the template as a separate package.
...
We believe that `react-native init` is a key entry point for new developers trying React Native, and we want to offer our best recommendation to the users invoking it.

So you can still use `react-native init` today to create new projects using the Community CLI & template today, but you’ll be seeing a warning as the follows:

![Init Deprecation](../static/blog/assets/0.75-deprecation.png)

Starting from December 31st 2024, the init command won’t create a project anymore, and you’ll have to either:
...
:::info
In order to offer a smoother migration experience, the `react-native@0.75.0` package is still depending on `@react-native-community/cli` but we’re planning on removing this dependency in a future version of React Native.
:::
...
With React Native 0.75, the auto-linking step is now running twice as fast on iOS and ~9 times faster on Android. You can read more about these improvements in this dedicated post [here](https://github.com/react-native-community/discussions-and-proposals/discussions)

<!-- TODO: Draft here but link needs to be updated: Autolinking in 0.75 blogpost draft -->

### Yoga 3.1 and Layout Improvements

Since we last shipped Yoga [version 3.0](https://reactnative.dev/blog/2024/04/22/release-0.74#yoga-30) in React Native 0.74, we kept on pushing many improvements and new layout capabilities for your applications. React Native 0.75 ships with Yoga 3.1 and you can learn more about what’s new in it in the official Yoga’s [release blog post](https://www.yogalayout.dev/blog/announcing-yoga-3.1).

One notable and highly requested feature is the support for % in various places, such as `gaps` and `translation`
...
With 0.75, the `gap`, `clomumnGap` and `rowGap`props described [here](https://reactnative.dev/docs/flexbox#row-gap-column-gap-and-gap) now support a string with a % as possible value.
...
Will be rendered as it follows:
...
Similarly to gaps properties, also the [`transform` prop](https://reactnative.dev/docs/transforms) can now accept % as values for the `translate` transformations.

So, for example, the following component would move the red square’s X coordinate by 100% of its width and the Y coordinate by 100% of its height:
...
The code is rendered into the following results:
...
Our focus in those last months has been in bridging the gap between the Old and New Architecture. Some examples of bugs and missing features we fixed are:
...
We also invite you to take part in the [New Architecture survey](https://t.co/ucAA58hnFu). This survey is key for us to collect precious feedback on the next steps for New Architecture rollout.

We also want to share a post we published in the New Architecture Working Group about [Android: Supporting UIManager in the New Architecture](https://github.com/reactwg/react-native-new-architecture/discussions/201). This post offers an API overview of the `UIManager` API and how it can help migration of more advanced apps & libraries between Old and New Architecture.
...
In the past, there has never been a recommended way from the framework to access the `jsi::Runtime` in native modules, and consumers would work around the framework to do so in risky ways. In 0.74, we introduced experimental APIs providing safe access to the `jsi::Runtime`, and we’re happy to announce their stability for 0.75.
...
If you’re on the UI thread and you need to access the runtime, we’ve also introduced an API for that called the `CallInvoker`. It consists of a simple API, `invokeAsync`, that will jump onto the JS thread to safely execute work with the runtime. These APIs are actually forward compatible.
...
Instead, you should use build-in react type `React.ElementRef` or, alternatively, the `View` type:
...
These are not breaking changes in 0.75 per-se, but we want to share how React Native 0.75 will be the last version of React Native to support minSdk 23 (Android 6.0) and minIOSVersion 13.4.
...
In 0.75, we are removing the remaining methods that were in core:

- `UIManagerModule.showPopupMenu()`
- `UIManagerModule.dismissPopupMenu()`

As alternative, please use the please use the `<PopupMenuAndroid />` component which lives in the [`@react-native/popup-menu-android`](https://www.npmjs.com/search?q=%40react-native%2Fpopup-menu-android) package.
...
- **Layout**
  - Support flex gap % values. This changes the parameters of some methods like `setGap`, `setRowGap` and `setColumnGap` from float to dynamic
...
React Native 0.75 contains over **1487 commits** from **165 contributors**. Thanks for all your hard work!
