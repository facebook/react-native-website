...
With 0.75, the `gap`, `columnGap` and `rowGap`props described [here](https://reactnative.dev/docs/flexbox#row-gap-column-gap-and-gap) now support a string with a % as possible value.
...
If you’re on the UI thread and you need to access the runtime, we’ve also introduced an API for that called the `CallInvoker`. It consists of a single API, `invokeAsync`, that will jump onto the JS thread to safely execute work with the runtime. These APIs are actually forward compatible.
...
We would like to announce two significant removals coming from the Community CLI: the commands `ram-bundle` and `profile-hermes` have been removed.
