...
- [React Native package renames](/blog/2023/05/30/0.72-metro-package-exports-symlinks#react-native-package-renames)
...
:::tip
See the newly released [blog post](https://reactnative.dev/blog/2023/05/08/package-exports-support) to learn more about this feature and our plans for stable rollout.
:::
...
Prior to this release, providing in an invalid style property in StyleSheet would result in a redbox. This is a high signal error that disrupts the developer workflow for a relatively low-risk error.
...
As well, LogBox stack traces now filter out internal Hermes bytecode frames that are not relevant to app users. [TODO get commit](TODO)

#### **Improved error output of the React Native CLI**

0.72 ships with React Native CLI v11.2.2 which includes improves to reduce duplication, clarify wording, reduce verbose stack traces, and add deep links to relevant docs in the following commands `init` , `run-android` and `run-ios`.
...
You can read the [0.72 updates for the New Architecture here](TODO). 
