...
We are also removing the deprecated `PropTypes` and `PushNotificationIOS` APIs. On Android, SDK 23 (Android 6.0) is now the minimum supported version.
...
<!-- TODO: Reinsert above - "Check out the new features [here](link_to_the_post)." -->

To learn more about the New Architecture, we moved the complete documentation from the React Native website to the [New Architecture Working Group](https://github.com/reactwg/react-native-new-architecture/tree/main/docs).
...
React Native 0.74 has a minimum Android SDK version requirement of 23 (Android 6.0). See our context for this change [here](https://github.com/react-native-community/discussions-and-proposals/discussions/740).
...
For example a newly created app with React Native 0.74 occupies ~13% less space on user device, resulting in ~4MB saved on device (see the attached screenshot).
...
**Android**
- Removal of `JSIModule*` from `FabricUIManagerProvider` ([#42059](https://github.com/facebook/react-native/pull/42059)).
...
- Deprecation of `UIManagerModule.showPopupMenu` and `UIManagerModule.dismissPopupMenu` ([#42441](https://github.com/facebook/react-native/pull/42441))
...
- Changed how the [`bundleURL`](https://github.com/facebook/react-native/pull/43994) is handled.
  ...
  - It should affect your app only if you were changing the `bundleURL` variable after the app is started. In this case, move the logic that updates the variable to the `bundleURL` function in the `AppDelegate`.
